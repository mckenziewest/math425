...
\newcommand{\secname}{Section 2.2: Groups}
...
\slide{
	\begin{thm}{2.2.1}
		If $(M,\cdot)$ is a monoid, then the set of all units $M^*$ is a group using the operation $\cdot$, called the \emph{unit group}.
	\end{thm}
	\vskip 2in
	\begin{exercise}
		Consider the following monoids. Determine their group of units.
		\bulletize{
			\setlength{\itemsep}{2em}
			\item $(\R,\cdot)$ has units $\R^*=$
			\item $(\Z,\cdot)$ has units $\Z^*=$
			\item $(\Z_4,\cdot)$ has units $\Z_4^*=$
		}
	\end{exercise}
	\vskip 2em
}
...
\slide{
	\begin{nb}
		\setstretch{1.5}
		We will say \textit{up to isomorphism} there is only one group of order 3.  
		...
		\enumalph{
			...
			\item $(\{1,\frac{-1+i\sqrt{3}}{2},\frac{-1-i\sqrt{3}}{2}\}\},\cdot)$ - the third roots of unity
		}
	\end{nb}
}
\slide{
	\begin{exercise}
		What about a group with 4 elements, is there 1 or are there more? (I leave two tables for exploration purposes.)
