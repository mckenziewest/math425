...
\begin{document}
	...
	\textbf{(Intro-3.2)} Let $f\colon \Z^+\to\R$ be defined recursively by $f(1)=1$ and $f(n+1)=\sqrt{2+f(n)}$ for all $n\in\Z^+$. Prove that $f(n)<2$ for all $n\in\Z^+$.
	
	\newpage
	Name: \underline{\hspace*{3in}}
	\vskip .25in
	
	\textbf{(Intro-3.3)} Prove that for all integers $n\geq 1$
	...
	\textbf{(Intro-3.4)} The Fibonacci numbers $f_n$, $n=1,2,3,\dots$ are defined by the formulas $f_1=1$, $f_2=1$, $f_n=f_{n-1}+f_{n-2}$ for all $n\geq 3$. 
	...
	\textbf{(Intro-3.5)} The Fibonacci numbers $f_n$, $n=1,2,3,\dots$ are defined by the formulas $f_1=1$, $f_2=1$, $f_n=f_{n-1}+f_{n-2}$ for all $n\geq 3$. 
	...
	\textbf{(Intro-3.6)} Let $A$ and $B$ be $2\times 2$ matrices with real entries such that $AB=BA$. Use induction to show that for all $n\geq 1$, we have $AB^n = B^nA$.
