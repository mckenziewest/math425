...
\end{thm}\begin{thm}{3.2.1}
	Let $R$ be a ring. The following are equivalent.  That is, either $R$ satisfies all of them or $R$ satisfies none of them.
	\enumarabic{
		\item If $ab=0$ in $R$, then $a=0$ or $b=0$.
		\item If $ab=ac$ in $R$ and $a\neq 0$, then $b=c$.
		\item If $ba=ca$ in $R$ and $a\neq 0$, then $b=c$.}
	Specifically, cancellation holds generally if and only if there are no zero divisors.
\end{thm}
\begin{thm}{3.2.2}
