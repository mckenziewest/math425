...
\newcommand{\secname}{Section 4.2: Factorization over a Field}
...
\slide{
	\begin{exa}
		\enumalph{\setlength{\itemsep}{.5in}
			\item $x^2+1$ is irreducible in $\R[x]$
			\item $x^2-2$ is irreducible over $\Q$
			\item $p=x^3+3x^2+x+2$ is irreducible over $\Z_5$
		}
		\vskip .5in
\end{exa}}
...
\slide{
	\begin{statementblock}{Unique Factorization Theorem (4.2.12)}
		...
		\enumarabic{\item $f=ap_1p_2\cdots p_m$, where $a\in F$ and $p_1,p_2,\dots,p_m$ are monic and irreducible  in $F[x]$.\item The factorization is unique up to the order of the factors.}
	\end{statementblock}
	\begin{nb}
...
{
	\begin{exercise}
		Complex conjugation is a ring homomorphism.  So let's assume that $z=a+bi$ is a root of a polynomial $f\in\R[x]$.  
...
}
{
	\begin{exercise}
		...
		\end{multline*}  Write this polynomial as a product of polynomials in $\Z[x]$.
	\end{exercise}
}
\newpage
{
...
{
	\begin{exercise}
		Show that $f=32x^3-51x^2-2x+25$ is irreducible over $\Q$.\\  (Hint: Check mod 3.)\vfill
	\end{exercise}
}
{
...
{
	\begin{exercise}
		Show that $x^5-3x^2+6x-12$ is irreducible in $\Q[x]$.\vskip 1in
	\end{exercise}
	\newpage
	\begin{exercise}
